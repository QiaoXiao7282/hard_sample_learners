...
def SNIP(net, keep_ratio, train_dataloader, device, args):

    # Grab a single batch from the training dataset
    if args.task == 'datacorr':
        data, attr, _ = next(iter(train_dataloader))
        if 'tiny' in args.data:
            inputs, targets = data, attr
        else:
            inputs, targets = data, attr[:, 0]
    else:
